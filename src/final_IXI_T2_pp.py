...
IXI = glob.glob('../Dataset/IXI2/**/*T2.nii.gz', recursive=True)
print(len(IXI))
...
#     fixed_image = sitk.ReadImage("./Dataset/MNI/MNI152_T1_1mm.nii.gz")  # MNI template
    ...
    sitk.WriteImage(moving_resampled,  "../temp/IXI_input_T2_KNN/" + name.split('/')[-1])
    sitk.WriteTransform(final_transform, "../temp/IXI_mask_T2_KNN/" + os.path.splitext(name.split('/')[-1])[0] + ".tfm")

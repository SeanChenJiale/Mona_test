...
SALD = glob.glob('../Dataset/SALD/**/*T1w.nii.gz', recursive=True)
...
for name in SALD:
    ...
    # sitk.WriteImage(corrected_image, "./PreProcessedData/SALD/" + name.split('/')[-1])
...
#     input_image = sitk.ReadImage("./Dataset/trial_SALD002-Guys-0828-T1_N4.nii.gz")
...
#     sitk.WriteImage(resampled_image, "./Dataset/trial_resample_SALD002-Guys-0828-T1_N4.nii.gz")
...
#     # Load images
#     fixed_image = sitk.ReadImage("./Dataset/MNI/MNI152_T1_1mm.nii.gz")  # MNI template
    moving_image = resampled_image # sitk.ReadImage("./Dataset/trial_resample_SALD002-Guys-0828-T1_N4.nii.gz")  # Your T1 MRI
    ...
    sitk.WriteImage(moving_resampled,  "../PreProcessedData/SALD_input_KNN/" + name.split('/')[-1])
    sitk.WriteTransform(final_transform, "../PreProcessedData/SALD_mask_KNN/" + os.path.splitext(name.split('/')[-1])[0] + ".tfm")
